...
class ModelGenerator:
    ...
    def build_UNet_3L(self, im_dim, clip_dim=None, ini_f=8, k_size=[3, 3]):
        """
        ...
        model = MaxPooling2D(pool_size=2, name='Pool1-2')(model)
        model = Conv2D(ini_f * 2, k_size, strides=1, padding='same', activation='relu',
                       kernel_initializer='he_normal', name='ConvE2-1')(model)
        # model = BatchNormalization(name='batchNormE2-1')(model)
        model = Conv2D(ini_f * 2, k_size, strides=1, padding='same', activation='relu',
                       kernel_initializer='he_normal', name='ConvE2-2')(model)
        # model = BatchNormalization(name='batchNormE2-2')(model)
        layer_stack.append(model)

        model = MaxPooling2D(pool_size=2, name='Pool2-3')(model)
        model = Conv2D(ini_f * 4, k_size, strides=1, padding='same', activation='relu',
                       kernel_initializer='he_normal', name='ConvE3-1')(model)
        ...
        model = Conv2D(ini_f * 2, k_size, strides=1, padding='same', activation='relu',
                       kernel_initializer='he_normal', name='ConvD1-2')(model)
        # model = BatchNormalization(name='batchNormD1-2')(model)

        # model = Conv2D(2, k_size, strides=1, padding='same', kernel_initializer='he_normal',
        #                name='output')(model)
        # model = Softmax(axis=-1)(model)
        model = Conv2D(1, k_size, strides=1, padding='same', activation='sigmoid', kernel_initializer='he_normal',
                       name='output')(model)
...
if __name__ == "__main__":
    m_g = ModelGenerator()
    model = m_g.build_UNet_3L((256, 256, 1))
    model.summary()
